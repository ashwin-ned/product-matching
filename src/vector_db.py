...
pc = Pinecone(api_key="pcsk_5Bvwek_P1i2zjgmuEPM6p7oZx6DePpyruAmwG3VoLwnMahrjjWmreiQi6E7W1oqTmuEmeo", environment=PINECONE_ENV)


def create_index():
    """
    Create a Pinecone index if it doesn't exist (handles ALREADY_EXISTS gracefully).
    """
    try:
        ...
        if e.status == 409:
            ...
            print(f"Index '{INDEX_NAME}' already exists (caught conflict)")
        else:
            raise
